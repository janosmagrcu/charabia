...
def create_mat(lmot, dico, nb_lettre, lg_max):
    ...
    for mot in lmot:
        ...
        if '-' in mot: #pour les mots composés : on les considère comme deux mots séparés
            ...
            if n == 1:
                mat[-1][id(mot[0])][0][-1] += 1 #proba d'un mot d'une lettre
            if n >= 2:
                mat[-1][id(mot[0])][0][id(mot[1])] += 1 #proba de la deuxième lettre
                for i in range(1, n-1):
                    mat[id(mot[i-1])][id(mot[i])][i][id(mot[i+1])] += 1 #proba de la lettre suivant la i-eme lettr
...
#fonction qui associe à une suite de 0, 1 ou 2 lettres la lettre suivant en fonction des probabilités calculées précedemment
    ...
    if len(l_1) == 1: #donne la deuxième lettre
        return (random.choices(alphabet, weights = mat[-1][id(l_1[0])][pos]))[0]
    if len(l_1) == 2:
        try: 
            lettre = (random.choices(alphabet, weights = mat[id(l_1[0])][id(l_1[1])][pos]))[0]
            return lettre
        except:
...
def create_word(mat, dico, alphabet, consonnes, voyelles,  choix_debut = None):
    ...
    if mot[:-1] in dico: #création de mots qui n'existent pas uniquement
        return create_word(mat, dico, alphabet, consonnes, voyelles,  choix_debut = choix_debut)
    return mot[:-1]
...
def create_verbs(mat,dico, alphabet, consonnes, voyelles, liste_term_troisieme_groupe, n): #fonction qui créer des pseudo-verbe à l'infinitif
    charabia = open('verbs_charabia.txt', 'w')
    for i in range(n):
        mot = create_word(mat, dico, alphabet, consonnes, voyelles)
...
def create_mat_charabiation():
    gram_a_charabie = gram_a_charabie()
    for gram in gram_a_charabie:
        liste_mot = list(lexicon_inv[gram].keys())
        dico, liste_lettres, nb_lettre, lg_max = variables_utiles_liste(liste_mot)
        matrice = create_mat(liste_mot, dico, nb_lettre, lg_max)
        name = str(gram)
