...
def charabieur_mot(mot_à_remplacer, gram, lexicon_inv, probas_inv, longueur = 0):
    """
    renvoie mot remplacé
    """
    
    # utiliser le code de Joseph avec en input les mots du lexicon de la même nature que le mot_à_remplacer
    # penser à gérer consonne/voyelle
    # et à redonner la casse (majuscule au début, tout en majuscule)
...
def charabieur(année_voulue, n_mots, proba_charabia):
    ...
    for i, (mot, gram) in enumerate(extrait_gram) :
        if len(mot)>4 and gram != 0 and gram not in gram_non_charabiées :
            ...
            if on_charabie :
                mot = charabieur_mot(mot, gram, lexicon_inv, probas_inv)
