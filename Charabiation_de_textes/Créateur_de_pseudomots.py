...
def create_word(mat, dico, alphabet, consonnes, voyelles,  choix_debut = None, starter = ''):
    if len(starter) < 2:
        mot = ''
        if len(starter) == 0:
    ...
    if mot[:-1] in dico: #création de mots qui n'existent pas uniquement
        return create_word(mat, dico, alphabet, consonnes, voyelles,  choix_debut = choix_debut, starter = starter)
    return mot[:-1]
...
consonnes, voyelles = cons_voy(alphabet)
print(create_word(mat, dico, alphabet, consonnes, voyelles, starter = 'h'))
